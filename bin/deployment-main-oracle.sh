...
##  Entrypoint
##
if dd if=/dev/zero of=/u02/swapfile bs=1024 count=31455; then
    echo "dd command executed successfully."
    chmod 0600 /u02/swapfile
    if mkswap /u02/swapfile; then
        echo "mkswap command executed successfully."
        if swapon /u02/swapfile; then
            echo "swapon command executed successfully."
        else
            echo "Error: swapon command failed."
        fi
    else
        echo "Error: mkswap command failed."
    fi
else
...
cp /tmp/tnsnames.ora /u02/Deployment/etc/
wait